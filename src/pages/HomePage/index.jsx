...
const HomePage = () => {
  ...
  const checkOwnerOf = async (value) => {
    if (isERC721) {
      const isValidOwner = await contractInstance.ownerOf(parseInt(value));
  ...
  const checkValidQuantity = async (quantity) => {
    if (quantity === 0) {
      setIdError({
        status: true,
    ...
    try {
      ...
      validQuantity = ethers.BigNumber.from(bigNumber._hex);
    } catch (error) {
      return false;
    }

    if (quantity > 0 && quantity <= validQuantity.toNumber()) {
      setFormError({
        ...formError,
        quantity: {
          status: false,
  ...
  const isDisableButton = !(formValues.address !== '' && formValues.id !== '');
      ...
      async (values) => {
        if (values[0] && values[1]) {
          if (isERC721) {
            ...
            const isValidQuantity = await checkValidQuantity(quantity);
